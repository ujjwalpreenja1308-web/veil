...
// Good enough for single-instance / low-traffic; Redis upgrades to distributed.
...
let upstashLimit: ((key: string) => Promise<LimitResult>) | null = null;

if (process.env.UPSTASH_REDIS_REST_URL && process.env.UPSTASH_REDIS_REST_TOKEN) {
  // Lazy import so the module doesn't crash when the packages aren't installed
  Promise.all([
  ...
  logger.info("[ratelimit] Using in-memory rate limiting (100 req/min per api_key)");
}
